...
class LogSoftmaxTest(tf.test.TestCase):
  ...
  def _npLogSoftmax(self, features):
    batch_dim = 0
    class_dim = 1
  ...
  def _testLogSoftmax(self, np_features, use_gpu=False):
    ...
    with self.test_session(use_gpu=use_gpu):
      tf_logsoftmax = tf.nn.log_softmax(np_features)
      out = tf_logsoftmax.eval()
    self.assertAllClose(np_logsoftmax, out)
  ...
  def _testAll(self, features):
    ...
    self._testLogSoftmax(features, use_gpu=True)

  def testNpLogSoftmax(self):
