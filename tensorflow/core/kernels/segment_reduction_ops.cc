    ...
    namespace functor {
        ...
        template<typename T, typename Index,
                ...
                void(*unsortedSegmentReductionOp)(Eigen::TensorMap<Eigen::Tensor<const T, 2, 1, long int>, 16> *,
                                                  ...
                                                  Index)>
        struct UnsortedSegmentBaseFunctor<CPUDevice, T, Index, unsortedSegmentInitializationOp, unsortedSegmentReductionOp> {
...
// specifying size of output.
    ...
    template<class T>
    inline void
    unsortedSegmentInitializationOpSum(Eigen::TensorMap<Eigen::Tensor<T, 2, 1, long int>, 16> *output_flat) {
      output_flat->setZero();
    }
    ...
    template<class T>
    inline void
    unsortedSegmentInitializationOpMax(Eigen::TensorMap<Eigen::Tensor<T, 2, 1, long int>, 16> *output_flat) {
    ...
    class UnsortedSegmentMaxOp
            : public UnsortedSegmentBaseOp<Device, T, Index, unsortedSegmentInitializationOpMax, unsortedSegmentReductionOpMax> {
    public:
        explicit UnsortedSegmentMaxOp(OpKernelConstruction *context)
                : UnsortedSegmentBaseOp<Device, T, Index, unsortedSegmentInitializationOpMax, unsortedSegmentReductionOpMax>(
                context) {}
    };

    template<typename Device, class T, class Index>
    class UnsortedSegmentSumOp
            : public UnsortedSegmentBaseOp<Device, T, Index, unsortedSegmentInitializationOpSum, unsortedSegmentReductionOpSum> {
    public:
        explicit UnsortedSegmentSumOp(OpKernelConstruction *context)
                : UnsortedSegmentBaseOp<Device, T, Index, unsortedSegmentInitializationOpSum, unsortedSegmentReductionOpSum>(
                context) {}
    };
...
#undef REGISTER_REAL_CPU_UNSORTED_KERNELS_ALL
  ...
  REGISTER_KERNEL_BUILDER(Name("UnsortedSegmentMax")                         \
                              ...
                              .TypeConstraint<type>("T")                     \
                              .TypeConstraint<index_type>("Tindices"),       \
                          UnsortedSegmentMaxOp<GPUDevice, type, index_type>);
