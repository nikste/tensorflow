...
namespace functor {
template <typename T>
struct LogSoftmaxFunctor<GPUDevice, T> {
  void operator()(const GPUDevice& d, typename TTypes<T>::ConstMatrix logits,
                  typename TTypes<T>::Matrix logsoftmax) {
    LogSoftmaxEigenImpl<GPUDevice, T>::Compute(d, logits, logsoftmax);
  }
};
}  // end namespace functor

// Instantiate the GPU implementation for float.
template struct functor::LogSoftmaxFunctor<GPUDevice, float>;
